  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const msg =
          data.error || data.message || `Server returned ${res.status}`;
  ...
  return (
    ...
    >
      <header className="absolute top-20 w-full flex justify-center pointer-events-none">
        <h1 className="text-5xl md:text-6xl font-semibold italic text-white select-none">
          Register Admin
        </h1>
      </header>
      ...
      <main className="w-full max-w-xl px-6 mt-20 md:mt-28">
        <div
          ...
          style={{
            boxShadow:
              "0 18px 30px rgba(2,6,23,0.35), inset 0 2px 0 rgba(255,255,255,0.6)",
          }}
        >
          <form className="max-w-lg mx-auto" onSubmit={handleSubmit} noValidate>
            ...
            <input
              ...
              className={`w-full h-12 md:h-14 rounded-[12px] px-6 placeholder:text-gray-400 focus:outline-none ${
                fullName ? "text-[#0b1221]" : "text-gray-700"
              }`}
            ...
            {/* Email */}
            <label className="block text-sm font-semibold text-black mt-6 mb-3">
              Email address
            </label>
            <input
              value={email}
              ...
              autoComplete="email"
              className={`w-full h-12 md:h-14 rounded-[12px] px-6 placeholder:text-gray-400 focus:outline-none ${
                email ? "text-[#0b1221]" : "text-gray-700"
              }`}
              style={inputBaseStyle}
            />
            {errors.email && (
            ...
            {errors.role && (
              <div className="text-sm text-red-600 mt-2">{errors.role}</div>
            )}
            ...
            <div className="relative">
              <input
                ...
                className={`w-full h-12 md:h-14 rounded-[12px] px-6 pr-16 placeholder:text-gray-400 focus:outline-none ${
                  password ? "text-[#0b1221]" : "text-gray-700"
                }`}
                style={inputBaseStyle}
              />
            ...
            <div className="relative">
              <input
                ...
                autoComplete="new-password"
                className={`w-full h-12 md:h-14 rounded-[12px] px-6 pr-16 placeholder:text-gray-400 focus:outline-none ${
                  confirm ? "text-[#0b1221]" : "text-gray-700"
                }`}
                style={inputBaseStyle}
              />
            ...
            {errors.confirm && (
              <div className="text-sm text-red-600 mt-2">{errors.confirm}</div>
            )}
            ...
            <button
              ...
              style={{
                background: ACCENT,
                boxShadow: "0 10px 20px rgba(255,87,34,0.25)",
                fontFamily: "Poppins, sans-serif",
              }}
            >
      ...
      <div
        ref={langRef}
        className="absolute right-6 bottom-6 text-white text-sm opacity-95"
        style={{ fontFamily: "Poppins, sans-serif", zIndex: 60 }}
      >
        <div className="relative inline-block">
          ...
          <div
            ...
            className={`absolute right-0 bottom-full mb-3 rounded bg-white text-[#0b1221] shadow-lg overflow-hidden ${
